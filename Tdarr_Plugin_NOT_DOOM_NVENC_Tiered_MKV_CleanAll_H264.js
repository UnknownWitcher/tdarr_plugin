...
const details = () => {
  return {
    id: "Tdarr_Plugin_NOT_DOOM_NVENC_Tiered_MKV_CleanAll_H264",
    Stage: 'Pre-processing',
    Name: "NOT-DOOM Tiered H264 MKV, remove audio & subtitles [NVENC]",
    Stage: "Pre-processing",
    Type: "Video",
    Operation: "Transcode",
    Description:
    ...
    Version: "2.0",
    Tags: "pre-processing,ffmpeg,nvenc h264",
    Inputs: [
      {
...
function buildVideoConfiguration(inputs, file, logger) {
  ...
  function videoProcess(stream, id) {
    ...
    if (stream.codec_name === "h264" && file.container === "mkv") {
      logger.AddSuccess("File is in H264 codec and in MKV");
      return;
    }
    ...
    if (stream.codec_name === "h264" && file.container !== "mkv") {
      ...
      logger.AddError("File is in H264 codec but not MKV. Will remux");
    }

    // remove png streams.
    if (stream.codec_name === "png") {
      configuration.AddOutputSetting(`-map -0:v:${id}`);
    } else if (stream.codec_name !== "h264") {  // Check if should Transcode.
      ...
      configuration.AddOutputSetting(
        `-c:v h264_nvenc -qmin 0 -cq:v ${cq} -b:v ${bitratetarget}k -maxrate:v ${bitratemax}k -preset medium -rc-lookahead 32 -spatial_aq:v 1 -aq-strength:v 8`
      );
      ...
      logger.AddError("Transcoding to H264 using NVidia NVENC");
    }
  }
